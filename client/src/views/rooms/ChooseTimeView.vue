  ...
  <div class="py-6 max-w-[400px] max-h-[800px]">
    ...
    <div class="flex-col space-y-2 rounded-none px-4">
      <div v-for="(active, index) in isActiveList" :key="index" class="flex items-center">
        ...
        <div
        class="flex items-center justify-center w-5 h-5 rounded-full bg-white border"
        >
      
        <div
        v-if="active"
        class="w-3 h-3 rounded-full bg-sky-400"
        ></div>
        </div>
        </button>
    ...
    <div class="flex rounded-lg p-4 items-center"> 
       <BasicButton 
       @click="goTokouhotipage"
       text="次へ  →" variant="primary" size="large"
       />
    </div>
  </div>
</template>